 ...
 * Description: 字符处理辅助类：主要处理字符的各种操作<br>
 * Copyright: Copyright (c)	2016 <br>
 * Create DateTime: 2016-11-25 上午10:21:09 <br>
...
public class StringHelpers {
	...
	/** The Constant chr. */
	public final static char[] chr = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h',
	...
	public static String autoAddZeroByRight(final int i, final int length) {
	 ...
	 * @return the string
	 */
	public static String Replace(final String source, final String oldtext,
		...
		for (int index = temp1.indexOf(oldtext); index >= 0; index = temp1
				.indexOf(oldtext)) {
	 ...
	 * @return
	 */
	public static String Replace(final String source, String oldtext,
			String newtext, String pattern) {
		if(notNull(newtext)){
			return Replace(source, oldtext, newtext);
		}else {
			int length = source.length();
			int os = source.indexOf(oldtext)+1;
			if(os==1||(os>1&&os<length)){
				oldtext+=pattern;
			}else if(os==length){
				oldtext=pattern+oldtext;
			}else {
				return source;
			}
	 ...
	 * @return the string
	 */
	public static String trimString(final String source, final String lststr,
			final int length, final boolean iscodelen) {
		...
		final int len = (endStr.length() < length) ? (length - endStr.length())
				: 2;
	 ...
	 * @return the string
	 */
	public static String toTrimHtmlVerticalString(final String source,
			final String lstline, final int length, final boolean iscodelen) {
		if ((source == null) || (source.trim().length() == 0) || (length <= 0)) {
			return source;
		}
	 ...
	 */
	public static String replace(final String str, final String pattern,
			String replace) {
	 ...
	 */
	public static final String getTreeLevelValue(final String oldValue,
			final String separateChar, final String replaceStr) {
	...
	/**
	 * 2006-1.11zhujw add 把数组字符串转化成'１','２','３'格式
	 * 
	 * @param fid
	 *            the fid
	 * @return the string
	...
	/**
	 * String >> char[] >> byte[] >> String 解决JSP中的中文问题.
	 * 
	 * @param source
	 *            the source
	 * @return the string
	...
	public static String fistCapital(String arg) {
		if (arg != null && !arg.trim().equals("")) {
			arg = arg.replaceFirst(String.valueOf(arg.charAt(0)), String
					.valueOf(arg.charAt(0)).toUpperCase());
		}
		return arg;
	}
	 ...
	 * @since 2011-9-14
	 */
	public static String join(List<?> datas, String parttern, String wrapChar,
			String field) {
	...
	public static void main(String[] args) {
		System.out.println(StringHelpers.getNumberString(1, 12));
	}
}
